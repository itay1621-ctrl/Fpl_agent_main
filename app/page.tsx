...
function PlayerInfoModal({ playerId, onClose, isDarkMode, isEnglish, teams }: { playerId: number, onClose: () => void, isDarkMode: boolean, isEnglish: boolean, teams: any }) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div 
        className={`w-full max-w-lg rounded-2xl p-6 shadow-2xl relative ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`} 
        onClick={e => e.stopPropagation()}
      >
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-gray-800 font-bold text-xl">✕</button>

        {loading ? (
          <div className="p-10 text-center animate-pulse font-bold">{isEnglish ? 'Loading Deep Stats...' : 'טוען נתונים מתקדמים...'}</div>
        ) : playerData ? (
          <div className="flex flex-col gap-6">
            <div className="flex items-center gap-4 border-b pb-4 border-gray-200 dark:border-gray-700">
              <img src={`https://fantasy.premierleague.com/dist/img/shirts/standard/shirt_${playerData.team_code}-66.webp`} className="w-16 h-auto drop-shadow-md" />
              <div>
                <h2 className="text-2xl font-black">{playerData.name}</h2>
                <div className="flex gap-2 text-sm font-bold opacity-80 mt-1">
                  <span className="bg-purple-100 text-purple-800 dark:bg-purple-900 dark:text-purple-200 px-2 rounded">
                    {playerData.pos_code === 1 ? 'GKP' : playerData.pos_code === 2 ? 'DEF' : playerData.pos_code === 3 ? 'MID' : 'FWD'}
                  </span>
                  <span>£{playerData.cost.toFixed(1)}M</span>
                  <span className="text-orange-500 flex items-center gap-1">🔥 Form: {playerData.form}</span>
                </div>
              </div>
            </div>

            <div>
              <h3 className="text-sm font-black text-gray-500 uppercase tracking-wider mb-3">{isEnglish ? 'Underlying Stats (Season)' : 'סטטיסטיקות עומק'}</h3>
              <div className="grid grid-cols-4 gap-2 text-center">
                <div className="p-2 bg-blue-50 dark:bg-gray-700 rounded-lg">
                  <p className="text-xs text-gray-500 font-bold">xG</p>
                  <p className="font-black text-blue-600">{playerData.xg}</p>
                </div>
                <div className="p-2 bg-purple-50 dark:bg-gray-700 rounded-lg">
                  <p className="text-xs text-gray-500 font-bold">xA</p>
                  <p className="font-black text-purple-600">{playerData.xa}</p>
                </div>
                <div className="p-2 bg-red-50 dark:bg-gray-700 rounded-lg">
                  <p className="text-xs text-gray-500 font-bold">xGC</p>
                  <p className="font-black text-red-600">{playerData.xgc}</p>
                </div>
                <div className="p-2 bg-emerald-50 dark:bg-gray-700 rounded-lg">
                  <p className="text-xs text-gray-500 font-bold">DEFCON</p>
                  <p className="font-black text-emerald-600">{playerData.defcon}</p>
                </div>
              </div>
            </div>

            <div>
              <h3 className="text-sm font-black text-gray-500 uppercase tracking-wider mb-3">{isEnglish ? 'Recent Gameweeks' : 'מחזורים אחרונים'}</h3>
              <div className="flex gap-2 justify-between">
                {playerData.history.map((h: any) => (
                  <div key={h.round} className="flex-1 flex flex-col items-center bg-gray-50 dark:bg-gray-750 p-2 rounded-lg border border-gray-100 dark:border-gray-600">
                    <span className="text-[10px] font-bold text-gray-400">GW {h.round}</span>
                    <span className={`font-black text-lg ${h.total_points >= 6 ? 'text-emerald-500' : h.total_points <= 2 ? 'text-red-500' : ''}`}>{h.total_points}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              <h3 className="text-sm font-black text-gray-500 uppercase tracking-wider mb-3">{isEnglish ? 'Upcoming Fixtures' : 'משחקים קרובים'}</h3>
              <div className="flex flex-col gap-2">
                {playerData.fixtures.map((f: any) => {
                  const isHome = f.is_home;
                  const diff = f.difficulty;
                  const oppId = isHome ? f.team_a : f.team_h;
                  const oppName = teams?.[oppId]?.short_name || 'UNK';
                  const bgFdr = diff <= 2 ? 'bg-emerald-500' : diff === 3 ? 'bg-gray-400' : diff === 4 ? 'bg-red-500' : 'bg-red-800';
                  
                  return (
                    <div key={f.id} className="flex justify-between items-center bg-gray-50 dark:bg-gray-750 p-2 px-3 rounded-lg border border-gray-100 dark:border-gray-600">
                      <span className="text-xs font-bold">GW {f.event}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-black">{oppName} ({isHome ? 'H' : 'A'})</span>
                        <span className={`text-[10px] text-white px-2 py-0.5 rounded font-bold ${bgFdr}`}>FDR {diff}</span>
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="p-10 text-center font-bold text-red-500">Failed to load player.</div>
        )}
      </div>
    </div>
  );
